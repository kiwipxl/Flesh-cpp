...
namespace msg {
    ...
    std::unique_ptr<MsgCallback> make_MID_callback          (std::function<ResponseCode()> callback, CMID mid, int num_callbacks = 1);
    ...
    std::unique_ptr<MsgCallback> make_any_MID_callback      (std::function<ResponseCode()> callback, int num_callbacks = 1);
};
